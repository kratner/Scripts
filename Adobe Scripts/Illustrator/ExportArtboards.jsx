...
function nyt_show_png_dialog() {
  ...
  nyt_png_dlg.resolutionEt = nyt_png_dlg.msgPnl.add(
    "edittext",
    undefined,
    "72"
  );
  nyt_png_dlg.resolutionEt.size = [50, 20];
  ...
  nyt_png_dlg.btnPnl.okBtn.onClick = function () {
    ...
    var exportFormat =
      nyt_png_dlg.exportFormatDropdown.selection.index === 0
        ? ExportType.PNG24
        : ExportType.JPEG;
    var resolution = parseInt(nyt_png_dlg.resolutionEt.text);
  ...
  for (var i = 0; i < num_artboards; i++) {
    ...
    if (!(artboardName.match(/^artboard/) || artboardName.match(/^\-/))) {
      ...
      nyt_png_dlg.progLabel.text =
        "Exported " + num_exported + " of " + num_to_export;
