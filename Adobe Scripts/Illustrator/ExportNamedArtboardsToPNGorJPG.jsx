...
function nyt_show_png_dialog() {
  ...
  nyt_png_dlg.progBar = nyt_png_dlg.msgPnl.add(
    "progressbar",
    undefined,
    0,
    100
  ...
  nyt_png_dlg.progLabel = nyt_png_dlg.msgPnl.add(
    "statictext",
    undefined,
    "Will export " +
      num_to_export +
  ...
  nyt_png_dlg.exportFormatDropdown = nyt_png_dlg.msgPnl.add(
    "dropdownlist",
    undefined,
    ["PNG", "JPEG"]
  );
  nyt_png_dlg.exportFormatDropdown.selection = 0;
  ...
  nyt_png_dlg.progLabel.size = [150, 20];

  nyt_png_dlg.qualityEt = nyt_png_dlg.msgPnl.add("edittext", undefined, "72");
  nyt_png_dlg.qualityEt.size = [50, 20];

  nyt_png_dlg.btnPnl = nyt_png_dlg.add("group", undefined, "");
  nyt_png_dlg.btnPnl.orientation = "row";

  nyt_png_dlg.btnPnl.cancelBtn = nyt_png_dlg.btnPnl.add(
    "button",
    undefined,
    "Cancel",
    { name: "cancel" }
  ...
  nyt_png_dlg.btnPnl.okBtn = nyt_png_dlg.btnPnl.add(
    "button",
    undefined,
    "Export",
    { name: "ok" }
  ...
  nyt_png_dlg.btnPnl.okBtn.onClick = function () {
    ...
    var exportFormat =
      nyt_png_dlg.exportFormatDropdown.selection.index === 0
        ? ExportType.PNG24
        : ExportType.JPEG;
    var resolution = parseInt(nyt_png_dlg.qualityEt.text);
    nyt_run_export(num_to_export, exportFormat, resolution);
  };

  nyt_png_dlg.show();
}

function nyt_run_export(num_to_export, exportFormat, resolution) {
  var num_exported = 0;
  var num_artboards = docRef.artboards.length;
  ...
  for (var i = 0; i < num_artboards; i++) {
    ...
    if (!(artboardName.match(/^artboard/) || artboardName.match(/^\-/))) {
      var destFile = new File(
        nyt_png_base_path + "/" + nyt_png_prefix + artboardName + ".png"
      );
      var options;

      if (exportFormat === ExportType.PNG24) {
        options = new ExportOptionsPNG24();
        options.artBoardClipping = true;
        options.matte = true;
      } else if (exportFormat === ExportType.JPEG) {
        options = new ExportOptionsJPEG();
        options.artBoardClipping = true;
        options.qualitySetting = resolution; // Set the quality value
      }

      options.horizontalScale = options.verticalScale = resolution;

      docRef.exportFile(destFile, exportFormat, options);

      num_exported++;
      var progress = (num_exported / num_to_export) * 100;
      nyt_png_dlg.progBar.value = progress;
      nyt_png_dlg.progLabel.text =
        "Exporting " + num_exported + " of " + num_to_export + " artboards...";
      nyt_png_dlg.update();
    }
  }
